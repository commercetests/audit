...
interface FallbackImageGalleryProps {
  images: string[];
}

export default function FallbackImageGallery({ images }: FallbackImageGalleryProps) {
  ...
  return (
    <div className="space-y-4">
      {/* Main selected image */}
      {selectedImage && (
        <div className="w-full h-48 backdrop-blur-md bg-black/20 rounded-xl overflow-hidden flex items-center justify-center border border-white/10 shadow-md">
          <img
            src={selectedImage}
            alt="Product image"
        ...
        <div className="flex space-x-2 overflow-x-auto pb-2 mt-2">
          {validImages.map((image, index) => (
            <button
              ...
              className={`w-14 h-14 flex-shrink-0 rounded-lg overflow-hidden border-2 shadow-md ${
